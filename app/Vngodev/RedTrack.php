    ...
    public static function crawl()
    {
        foreach (User::all() as $key => $user) {
            foreach ($user->campaigns as $index => $campaign) {
                ...
                $url = 'https://api.redtrack.io/report?api_key=' . $user->trackers->first()->api_key . '&date_from=' . $date . '&date_to=' . $date . '&group=sub6,hour_of_day&sub6=' . $campaign->campaign_id . '&tracks_view=true';
